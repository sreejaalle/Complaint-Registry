...
app.delete("/deleteComplaint/:id", async (req, res) => {
  try {
    ...
    if (complaint.status !== "completed") {
      return res.status(403).json({ error: "Only completed complaints can be deleted" });
    }

    await ComplaintSchema.findByIdAndDelete(complaintId);
    await AssignedComplaint.deleteMany({ complaintId: complaintId });
...
app.post("/assignedComplaints", async (req, res) => {
  try {
    ...
    const updatedComplaint = await ComplaintSchema.findByIdAndUpdate(
      complaintId,
      { assignedTo: agentName, agentId: agentId },
      { new: true }
    );
    ...
    res.status(200).json({ message: "Agent assigned successfully", updatedComplaint });
  } catch (error) {
    console.error(error);
...
app.put("/updateComplaint/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { assignedTo, agentId, status } = req.body;
...
// // Delete complaint only if status is "completed"
// app.delete("/deleteComplaint/:id", async (req, res) => {
//   try {
//     const complaintId = req.params.id;
//     const complaint = await ComplaintSchema.findById(complaintId);
...
//     await ComplaintSchema.findByIdAndDelete(complaintId);
//     await AssignedComplaint.deleteMany({ complaintId: complaintId }); // clean up assigned list if needed
...
//   }
// });
// Assigned complaint by admin and update original complaint
